...
def _load_deferred_track_d():
    import deferred_track_d

    return deferred_track_d


# Keep deferred Track D entrypoints importable for historical tooling, but keep
# their implementation out of the active proof-loop module.
def run_python_track_d(root: pathlib.Path):
    deferred_track_d = _load_deferred_track_d()
    return deferred_track_d.run_python_track_d(sys.modules[__name__], root)


def run_rust_track_d(root: pathlib.Path):
    deferred_track_d = _load_deferred_track_d()
    return deferred_track_d.run_rust_track_d(sys.modules[__name__], root)
